      ...
      <ul className='flex flex-col gap-2.5 list-none p-0'>
        {links.map((link, i) => (
          <li key={i} className='group flex items-start'>
            {/* Note: In a real app, replace '#' with a dynamic href property */}
            <Link
              href='#'
              className='text-left text-xs text-[#555555] leading-relaxed border-l border-transparent pl-2.5 transition-all group-hover:text-[#1E3A2F] group-hover:border-[#1E3A2F]'
            >
              {link}
            </Link>
          </li>
